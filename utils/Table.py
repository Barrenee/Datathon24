...
class Table(dict):
    ...
    def initialize(self, allgroups: List[Group], preferences: callable):
        for key in self.self.keys_:
            for group in allgroups:
                self.group_prefs[group] = []
                if key in group.preferences:
                    self.table[key].append(group)
                    self.group_prefs[group].append(key)
        return self.table
    
    def update(self, group1: Group, group2: Group, new_keys):
        """
